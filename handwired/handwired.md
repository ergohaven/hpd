...
После припаивания контроллеров к половинкам клавиатуры их рекомендуетмя проверить на корректную работу.
Для начала нужно прошить обе половинки: подключите кабель USB-C от ПК к контроллеру, скопируйте файл прошивки для HPD v2 (no modules) из [keymap_hub](https://github.com/ergohaven/keymap_hub) в корень папки контроллера RPI-RP2. Повторите те же действия с другой половинкой. 
После прошивки соедените половинки между собой и подключите к ПК, откройте [Vial](https://eh.works/vial) и проверьте половинки в **Тестере матрицы (Matrix tester)**
